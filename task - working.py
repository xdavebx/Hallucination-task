...
class ConditioningTrial(object):
    ...
    def __init__(self, task, trial_number, stimulation=True, max_voltage=20, baseline_stimulation=True):
        ...
        self.give_baseline_stimulation = False
        self.baseline_interval = self.task.config['stimulation']['baseline_interval']
        self.baseline_mean = self.task.config['stimulation']['baseline_mean']
        self.baseline_sd = self.task.config['stimulation']['baseline_sd']
    ...
    def baseline_stimulation(self, interval=2, mean=0, sd=1, current_volt=20):

        """
        Adjusts voltage to produce fluctuations at baseline (drawn from a Gaussian distribution)
    ...
    def ramp_stumuli(self, t, catch, start=0, max_voltage=50, start_volt=0, end_volt=None):
        ...
        start_volt: Voltage to start the ramp from
        end_volt: Voltage to return to after stimulation
        ...
        if start < t < start + self.ramp_up:
            ...
            voltage_diff = max_voltage - start_volt
            ...
            # Heat
            self.task.trigger_shock(start_volt + percentage * voltage_diff)
        ...
        # Ramp down
            ...
            # Calculate required voltage increase
            voltage_diff = end_volt - max_voltage
    ...
    def run(self, confidence=False, end_on_response=True, catch=False, save_data=True):
        ...
        current_volt = self.baseline_mean
        ...
        while continue_trial:
            ...
            if t < self.fixation:
                ...
                if self.give_baseline_stimulation:
                    current_volt = self.baseline_stimulation(self.baseline_interval,
                                                             self.baseline_mean,
                                                             self.baseline_sd,
                                                             current_volt)

            # Ramp up / down visual and heat stimuli
            self.ramp_stumuli(t, catch, start=self.fixation, start_volt=current_volt, end_volt=current_volt,
                ...
                # Otherwise, show the confidence rating scale or just get responses and move on for binary trials
                    ...
                    for key_event in keys:
                        ...
                        if key_event.type == EventConstants.KEYBOARD_PRESS and \
                            ...
                            # End the trial if we're not getting  ratings
            ...
            # End trial
            if t > self.fixation + self.ramp_up + self.hold + self.ramp_down + self.task.config['durations']['response_time']:
                continue_trial = False
